...
Every supported line or shape can use solid, dashed, dotted, dash–dot, or long-dash ink. The Add vertex action inserts an exact movable point on a selected line, arrow, or curve; line insertion preserves arrowheads at the original ends, while moved curve points carry their attached handles by the same delta so neighboring arc shapes stay locally stable. Create vertices remains available for curves, outlined shapes, and mixed-path selections while line-only selections use Add vertex. Groups can be nested to any practical depth; each Ungroup removes only the current level and restores the smaller groups below it. Lock size remains available for every selection, suppresses ordinary resizing, preserves line length, constrains shared joints, and retains rigid-group dimensions. Connected rigged groups move together as a character. The local Board library saves named selections with visual previews and preserves embedded images, nested groups, internal vertices, shared joints, object locks, and size locks when a saved asset is inserted again. Download selection exports the same relationship-preserving package as a `.vp-character.json` file that can be dropped back onto the board.
...
Selected images now support non-destructive source-coordinate crops, aspect presets, fit/fill/original-size actions, numeric and handle rotation, replacement, and horizontal or vertical flips. Complete selections and rigged groups flip around their collective center while retaining groups, vertices, joints, locks, arrow direction, and crop state; text remains readable unless glyph mirroring is explicitly enabled. Floor Plan mode organizes modular Structures, Maintenance, Furniture, and Rooms catalogs, with utilities and appliances, two-point dimensions, editable hover labelers, signed non-wrapping levels, snapping, alignment guides, and starter rooms. Its Floor Plan-only Tools include editable 3–9-sided outer-building, inner-building, outside-wall, and outside-fence primitives. Building runs automatically become thicker outer-shell or thinner interior walls; coincident and partially overlapping runs are split and deduplicated, while nearby doors and windows attach to and slide along eligible walls. Its 30 authored vector defaults include the complete door, window, stair, dimension, appliance, bathroom, cabinet, laundry, bed, sofa, table, nightstand, and pool/hot-tub set. Labelers combine an editable detection box, vertex-controlled arrow, and boxed text. Dimensions and hover labels have board-wide visibility controls. Selected vector artwork can be saved, inserted, renamed, replaced, removed with the configured password, or used to override a built-in item; hidden or customized defaults can be restored. The general toolbar also includes a persistent-color vector bucket that fills clicked closed objects or connected line-and-curve regions without selecting them, fill removal, a combined Line/Arrow chooser, optional double-ended arrows, brush-local vector erasing that never changes locked objects, and editable grouped joints that merge identities when dropped together. Every inserted building block remains ordinary editable Board geometry. Image-backed assemblies continue to use the general Board library.
...
Starts with a named trip and a five-part brief: where and when, transportation, staying and eating, activities, and departure plus the next destination. The selected trip then owns the existing month calendar, overhead Select and Add Event tools, paged selected dates, and 24-hour day timelines. Existing version-1 events remain available under Unassigned itinerary.
