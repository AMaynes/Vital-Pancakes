...
Selected images now support non-destructive source-coordinate crops, aspect presets, fit/fill/original-size actions, numeric and handle rotation, replacement, and horizontal or vertical flips. Complete selections and rigged groups flip around their collective center while retaining groups, vertices, joints, locks, arrow direction, and crop state; text remains readable unless glyph mirroring is explicitly enabled. Floor Plan mode adds scale-aware walls, doors, windows, stairs, labels, dimensions, common room symbols, snapping, alignment guides, starter rooms, and a blank house shell. Selected vector artwork can be saved as either a named floor-plan element or room template, inserted again, renamed, replaced from a new selection, removed, or used to override a built-in item; hidden or customized defaults can be restored. Every inserted building block remains ordinary editable Board geometry. Image-backed assemblies continue to use the general Board library.
