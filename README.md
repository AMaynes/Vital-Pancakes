...
Selected images now support non-destructive source-coordinate crops, aspect presets, fit/fill/original-size actions, numeric and handle rotation, replacement, and horizontal or vertical flips. Complete selections and rigged groups flip around their collective center while retaining groups, vertices, joints, locks, arrow direction, and crop state; text remains readable unless glyph mirroring is explicitly enabled. Floor Plan mode organizes modular Structures, Furniture, and Rooms catalogs, with utilities and appliances, two-point dimensions, hover labels, switchable room/floor designators, snapping, alignment guides, and starter rooms. Dimensions and hover labels have board-wide visibility controls. Selected vector artwork can be saved, inserted, renamed, replaced, removed with the configured password, or used to override a built-in item; hidden or customized defaults can be restored. The general toolbar also includes fill color, bucket painting, and fill removal for closed objects. Every inserted building block remains ordinary editable Board geometry. Image-backed assemblies continue to use the general Board library.
