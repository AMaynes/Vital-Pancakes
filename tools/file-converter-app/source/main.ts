...
import { compareFileBytes, type ByteVerificationResult } from "./byteVerification.ts";
...
const ui = {
  ...
  byteCheck: document.querySelector("#byte-check") as HTMLInputElement,
...
function findCompatibleOption(
  option: { format: FileFormat, handler: FormatHandler },
  direction: "from" | "to"
) {
  if (option.format[direction]) return option;
  return allOptions.find(candidate =>
    candidate.format[direction]
    && candidate.format.mime === option.format.mime
    && candidate.format.format === option.format.format
  );
}

function formatByteVerification(result: ByteVerificationResult): string {
  if (result.matches) {
    return `<p class="byte-check-result byte-check-pass">
      <b>Byte-for-byte check passed.</b> ${result.comparedBytes.toLocaleString()} bytes matched exactly.
    </p>`;
  }

  if (result.reason === "file-count") {
    return `<p class="byte-check-result byte-check-fail">
      <b>Byte-for-byte check failed.</b> Expected ${result.expected} returned file(s), received ${result.actual}.
    </p>`;
  }

  if (result.reason === "file-length") {
    return `<p class="byte-check-result byte-check-fail">
      <b>Byte-for-byte check failed.</b> File ${Number(result.fileIndex) + 1} changed from
      ${result.expected?.toLocaleString()} to ${result.actual?.toLocaleString()} bytes.
    </p>`;
  }

  return `<p class="byte-check-result byte-check-fail">
    <b>Byte-for-byte check failed.</b> File ${Number(result.fileIndex) + 1} first differs at byte
    ${result.mismatchOffset?.toLocaleString()}.
  </p>`;
}

async function verifyRoundTrip(
  originalFiles: FileData[],
  convertedFiles: FileData[],
  inputOption: { format: FileFormat, handler: FormatHandler },
  outputOption: { format: FileFormat, handler: FormatHandler }
): Promise<string> {
  ui.popupBox.innerHTML = `<h2>Checking every byte...</h2>
    <p>Converting the result back to ${inputOption.format.format.toUpperCase()}.</p>`;

  const roundTripSource = findCompatibleOption(outputOption, "from");
  const roundTripTarget = findCompatibleOption(inputOption, "to");
  if (!roundTripSource || !roundTripTarget) {
    return `<p class="byte-check-result byte-check-unavailable">
      <b>Byte-for-byte check unavailable.</b> This format pair has no complete return route.
    </p>`;
  }

  const roundTrip = await window.tryConvertByTraversing(
    convertedFiles.map(file => ({ ...file, bytes: new Uint8Array(file.bytes) })),
    roundTripSource,
    roundTripTarget
  );
  if (!roundTrip) {
    return `<p class="byte-check-result byte-check-unavailable">
      <b>Byte-for-byte check unavailable.</b> The converter could not complete the return route.
    </p>`;
  }

  ui.popupBox.innerHTML = `<h2>Checking every byte...</h2>
    <p>Comparing the returned files with the originals.</p>`;
  const result = await compareFileBytes(originalFiles, roundTrip.files);
  return formatByteVerification(result);
}
...
ui.convertButton.onclick = async function () {
  ...
  try {
    ...
    const verificationOriginals: FileData[] = [];
    for (const inputFile of inputFiles) {
      const inputBuffer = await inputFile.arrayBuffer();
      const inputBytes = new Uint8Array(inputBuffer);
      inputFileData.push({ name: inputFile.name, bytes: inputBytes });
      verificationOriginals.push({
        name: inputFile.name,
    ...
    const sameFormat = inputFormat.mime === outputFormat.mime
      && inputFormat.format === outputFormat.format;
    const output = sameFormat
      ? { files: inputFileData, path: [inputOption] }
      : await window.tryConvertByTraversing(inputFileData, inputOption, outputOption);
    if (!output) {
      window.hidePopup();
    ...
    if (ui.byteCheck.checked) {
      try {
        byteVerification = sameFormat
          ? formatByteVerification(await compareFileBytes(verificationOriginals, output.files))
          : await verifyRoundTrip(
              verificationOriginals,
              output.files,
              inputOption,
              outputOption
            );
      } catch (error) {
        console.error("Byte-for-byte verification failed:", error);
        byteVerification = `<p class="byte-check-result byte-check-unavailable">
          <b>Byte-for-byte check could not finish.</b> The converted file is still available.
        </p>`;
      }
    }
