...
`visual-board-image.mjs` stores crop rectangles in source-image coordinates and supplies fit, fill, reset, replacement, and draw geometry without iterative recompression. `visual-board-transform.mjs` mirrors complete selections and only their fully selected rig joints while preserving IDs, locks, vertices, arrows, architectural areas and symbols, crop state, and readable text. `visual-board-floor-plan.mjs` creates walls, doors, swing arcs, windows, dimensions, room symbols, and starter rooms through the existing object and group schema. `visual-board-floor-plan-templates.mjs` stores vector-only character packages inside separate floor-plan element and room-template libraries, supports custom items and built-in overrides, and keeps every insertion editable. The Board UI can create, rename, replace, insert, remove, and restore both kinds of building block; version 17 adds user-owned elements while preserving version 15 templates and version 16 exact multi-point curves.
...
`visual-board-ai-adapter.mjs` exposes exact curve-point insertion, key-vertex reinitialization, and shared line/curve vertex commands alongside architecture commands for polygon/curved areas, wall segments and connected wall paths, opening cuts, symbols, labels, dimensions, style systems, material assignment, layer replacement, consent-gated local reference overlays, inspection, quality validation, and the complete custom floor-plan element/template lifecycle. Building-block commands save explicit vector targets, keep character contents out of routine context summaries, and reject image assets. Commands validate nested fields, operate on cloned state, support same-batch client keys, preview without saving, and commit an approved mixed batch as one undoable change. ChatGPT/Codex, manual JSON, MCP, and WebLLM share this adapter. The adapter and renderer never plan, infer, furnish, reroute, or improve a design; the calling model supplies every coordinate, dimension, style, material, symbol, layer, and z-index.
