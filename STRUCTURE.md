...
│   ├── visual-board-curves.mjs — Preserves legacy arcs and owns exact multi-vertex Bezier insertion, sampling, and transforms.
│   ├── visual-board-curves.test.mjs — Verifies path-preserving insertion, editable knots, sampling, and transforms.
...
`visual-board-ai-adapter.mjs` exposes exact curve-point insertion and shared line/curve vertex commands alongside architecture commands for polygon/curved areas, wall segments and connected wall paths, opening cuts, symbols, labels, dimensions, style systems, material assignment, layer replacement, consent-gated local reference overlays, inspection, quality validation, and the complete custom floor-plan template lifecycle. Template commands save explicit vector targets, keep character contents out of routine context summaries, and reject image assets. Commands validate nested fields, operate on cloned state, support same-batch client keys, preview without saving, and commit an approved mixed batch as one undoable change. ChatGPT/Codex, manual JSON, MCP, and WebLLM share this adapter. The adapter and renderer never plan, infer, furnish, reroute, or improve a design; the calling model supplies every coordinate, dimension, style, material, symbol, layer, and z-index.
