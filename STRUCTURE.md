...
`pdf-signer.html` hosts the compact accordion workflow. `pdf-signer.js` loads a chosen PDF with bundled PDF.js; manages selectable, edge-draggable, lockable, copyable, pixel-sized, four-corner-resizable signatures, dates, auto-fitting styled fillable text fields, vector marks, and white-out overlays; and exports them with PDF-Lib. `pdf-tool-export.mjs` turns fillable areas into genuine editable AcroForm text fields, preserves field-level typography, and renders marks and white-out areas as vectors. The tool only adds overlays and never detects or removes original PDF content. `pdf-signer-placements.mjs` validates immutable placement creation, duplication, editing, locking, and deletion for both UI and AI commands, with adjacent Node test suites.
